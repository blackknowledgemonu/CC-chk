...
#———–———–———–———–———–———#
bot = telebot.TeleBot("8306760037:AAFK6NtvAmWohWmfm-h1uiVSK0Cq7_ehKeM", parse_mode='html')
admin_id = 5621614605
...
CHANNEL_ID = -1002586398039   # 👈 لازم تجيب ID القناة الخاصة
...
def handle_paypal_file_upload_venom(message):
    ...
    for card in cards:
        ...
        # تحديث العدادات أثناء الفحص VENOM STYLE
        try:
            bot.edit_message_reply_markup(
                chat_id=message.chat.id,
...
"""
    ...
    # إرسال الملخص النهائي VENOM STYLE
    try:
        bot.edit_message_text(
...
<a href='tg://user?id=5621614605'>𝗼𝘄𝗻𝗲𝗿</a>'''
...
def daily_gift_callback(call):
    ...
    bot.send_message(call.message.chat.id, f"🎉 𝗗𝗮𝗶𝗹𝘆 𝗴𝗶𝗳𝘁 𝗰𝗹𝗮𝗶𝗺𝗲𝗱! +𝟱 𝗰𝗿𝗲𝗱𝗶𝘁𝘀.\n💳 𝗡𝗲𝘄 𝗯𝗮𝗹𝗮𝗻𝗰𝗲: {credits_data[user_id]['credits']}")
